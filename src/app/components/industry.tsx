...
export default function IndustryCoverage() {
    ...
    return (
        <section className="bg-[var(--dark-grey)] text-[var(--white)] backdrop-brightness-50 flex flex-col items-center md:p-8 p-4">
            ...
            <div className="flex flex-wrap justify-center md:mt-12 mt-4">
                {coverage.map((coverageItem, index) => {
                    return (
                        <div className="w-32 h-20 md:w-64 md:h-36 rounded-md outline outline-offset-2 outline-[var(--green)] flex justify-center items-center bg-black m-4"
                            key={index}>
                            <p className="text-center md:text-xl text-sm">{coverageItem}</p>
                        </div>
                    );
                })}
            </div>
